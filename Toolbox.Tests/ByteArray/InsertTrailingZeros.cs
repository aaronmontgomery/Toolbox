...
namespace ByteArray
{
    ...
    public static partial class InsertTrailingZeros
    {
        ...
        {
            Assert.Throws<System.ArgumentException>(() => Toolbox.ByteArray.InsertTrailingZeros(byteArray, length));
        }

        [TestCase(null, 2)]
        public static void InsertTrailingZeros_WithNull_ThrowsArgumentNullException(byte[] byteArray, int length)
        {
            Assert.Throws<System.ArgumentNullException>(() => Toolbox.ByteArray.InsertTrailingZeros(byteArray, length));
        }
    }
}
