...
namespace ByteArray
{
    ...
    public static partial class InsertLeadingZeros
    {
        ...
        {
            Assert.Throws<System.ArgumentException>(() => Toolbox.ByteArray.InsertLeadingZeros(byteArray, length));
        }

        [TestCase(null, 2)]
        public static void InsertLeadingZeros_WithNull_ThrowsArgumentNullException(byte[] byteArray, int length)
        {
            Assert.Throws<System.ArgumentNullException>(() => Toolbox.ByteArray.InsertLeadingZeros(byteArray, length));
        }
    }
}
